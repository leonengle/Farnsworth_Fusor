    ...
    def _client_loop(self):
        while self.running:
            try:
                ...
                logger.info(
                    f"Connecting to target data server at {self.target_ip}:{self.target_port}"
                )
                ...
                if self.running:
                    if e.errno == 10051:  # Windows: Network unreachable
                        logger.error(
                            f"Network unreachable - cannot reach {self.target_ip}:{self.target_port}"
                        )
                    elif e.errno == 10061:  # Windows: Connection refused
                        logger.warning(
                            f"Connection refused - is target running on port {self.target_port}?"
                        )
                    else:
