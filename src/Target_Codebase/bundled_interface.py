...
class BundledInterface:
    ...
    def send_motor_command(self, motor_id: int, command: str, *args) -> Optional[str]:
        ...
        arduino_cmd = self.validator.build_motor_command_string(
            motor_id, command, *args
        )
        if not arduino_cmd:
            logger.error(
                f"Motor command validation failed for motor {motor_id}, command {command}"
            )
            return None
    ...
    def send_motor_object(self, motor_id: int, percentage_or_power: float) -> bool:
        ...
        is_valid, error = self.validator.validate_motor_degree_object(
            component_name, motor_degree
        )
        if not is_valid:
        ...
        if self.arduino_interface.send_motor_object(component_name, motor_degree):
            logger.debug(
                f"Sent motor object: {component_name}, degree: {motor_degree} (from {percentage_or_power}%)"
            )
            return True
    ...
    def send_analog_to_arduino(self, label: str, value) -> bool:
        ...
        try:
            ...
            if not arduino_cmd:
                logger.error(
                    f"Analog command validation failed for label {label}, value {value}"
                )
                return False
